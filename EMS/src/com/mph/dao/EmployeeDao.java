...
public void insertUsingProcedure(Employee emp) {
	...
	try {
		...
		cs.execute();
		System.out.println("Procedure succefully executed and record inserted!!");
	}catch(SQLException e) {
		e.printStackTrace();
	}
}
...
public void batchUpdate()
{
	...
	try {
		st = con.createStatement();
		//st.addBatch("update mphemp set ename ='Mohann' where eid=1");
		st.addBatch("update mphemp set eid=12 where ename ='ignesh'");
		st.addBatch("update mphemp set ename ='Varun' where eid=12");
		st.addBatch("update mphemp set ename ='Sahitha' where eid=11");
		//st.addBatch("update mphemp set ename ='Rajaa' where eid=5");
